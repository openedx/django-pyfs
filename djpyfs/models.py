...
class FSExpirations(models.Model):
    ...
    def __str__(self):
        if self.expires:
            return f"{os.path.join(self.module, self.filename)} Expires {str(self.expiration)}"
        else:
            return f"{os.path.join(self.module, self.filename)} Permanent ({str(self.expiration)})"
